...
**Status.** This contract is the definition of the stage. The board's stage machine does
not know `ticketed` yet: the road in [EXECUTION.md](./EXECUTION.md), the stage table in
[API.md](./API.md), CONTEXT.md's Stage entry, and `bin/pipeline.mjs` all still run
`spec → grilled → development`. Until the stage lands on the board, the card formally
stays in `grilled` while the ticket set is written, and the checklist in §4 gates the
move to `development`. Landing the stage means amending, in one change: EXECUTION.md and
API.md (road + stage table), CONTEXT.md (Stage entry), WATCHDOG.md and TELEGRAM.md stage
lists, `bin/pipeline.mjs`, and the `assign-subscription` contract — the owner's
subscription answer is valid from `grilled` today and must become valid from `ticketed`,
so that its auto-advance to `development` fires only after the §4 checklist is green.
