...
10. Known environment noise (never a finding, never a stop): seller photos from `img.autopase.lv` blocked in the automation browser (`ERR_BLOCKED_BY_RESPONSE`, Cloudflare challenge — real browsers load them) until the WAF exception covers that host; the intermittent React #418 on public results pages (#2024). Record them as `environment noise` / `known #2024` and walk on.
