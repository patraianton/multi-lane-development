...
- **CI has two modes** (owner rule 2026-09-12, product PR #2274): a PR WITHOUT the `full-ci` label always
  runs the scoped `pr-ci` (~7–10 min) — no file pattern may escalate it. The full pipeline runs only on the
  label, reports as `pr-ci-full`; main does not rebuild a head that already passed it. A merge needs BOTH
  green on the current head; a red or pending `pr-ci-full` is *waiting* — no fix, no stuck. The board alarms
  the owner when an unlabelled PR's `pr-ci` job runs past 15 min — the rule slipping, not the queue.
