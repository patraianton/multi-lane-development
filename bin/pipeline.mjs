...
function unitPlan(cards, sprints) {
  ...
  for (const [sprintId, s] of sprints) {
    ...
    for (const u of [...units, ...qa]) {
      ...
      const verdictOnHead = u.pr && Object.hasOwn(u.pr, 'verdictOnHead')
        ? u.pr.verdictOnHead
        : u.pr?.verdict;
      const verdictAt = verdictOnHead?.at ?? null;
      ...
      const noGo = card?.stage === 'ci_pr' && verdictOnHead?.go === false ? verdictOnHead : null;
