...
test('notifyReady sends one exact line to ownerChatId and never falls back to the group', async () => {
  ...
  try {
    ...
    configureTelegram({ ...BASE_CONFIG, ownerChatId: '' });
    await assert.rejects(notifyReady(CARD), /no ownerChatId/);
  } finally {
    configureTelegram(null);
    globalThis.fetch = originalFetch;
  }

  assert.deepEqual(payloads.map(payload => payload.chat_id), ['4242']);
  ...
  assert.ok(payloads.every(payload => !payload.text.includes('127.0.0.1')),
